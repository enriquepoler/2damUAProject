...
class SQLite_consulter:
    ...
    def insert_lap_into_db(self, patient_dni, patient_name, patient_surname, total_time, lap1, lap2, lap3, score, status, anotations, user):
        ...
        self.connection.execute("INSERT INTO laps (patient, name, surname, totalTime, lap1, lap2, lap3, score, status, anotations, user) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",(patient_dni, patient_name, patient_surname, total_time, lap1, lap2, lap3, score, status, anotations, user))
        self.connection.commit()
    ...
    def get_patient_info(self, patient_name_surname):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT dni, name, surname, height, weight FROM patients WHERE name = ? and surname = ?", (patient[0], patient[1]))
        rows = cursor.fetchall()
    ...
    def delete_patient(self, patient_dni):
        ...
        self.connection.execute("DELETE FROM patients WHERE dni = ?",([patient_dni]))
        self.connection.commit()
