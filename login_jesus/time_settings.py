...
class Time_settings(QDialog):
    ...
    def fill_cb_status(self):
        ...
        self.combo_box_status.currentIndexChanged.connect(self.selection_change_status)
    ...
    def selection_change_status(self):
        ...
        if(self.combo_box_status.currentText() != "Selecciona un estat" and self.combo_box_status.currentText() != ""):
            ...
            status_info = self.sqlite.get_status_info(self.combo_box_status.currentText())
    ...
    def sure_to_save(self, selection):
        ...
        if(self.contador == 1):
            if(selection.text() == "&Yes"):            
                ...
                try:
                    self.total_new_old_info = float(self.total_lap_text_edit.text())
                except:
                ...
                if(str(self.lap1_new_info) != "" and str(self.lap2_new_info) != "" and str(self.lap3_new_info) != ""):
                    ...
                    self.text_read_only()
                    self.combo_box_status.setEnabled(True)
                    self.sqlite.modify_status(self.status, self.lap1_new_info, self.lap2_new_info, self.lap3_new_info, self.total_new_old_info)
                    self.fill_cb_status()   
                else:
